...
\newcommand{\coverphoto}[2]{
	\postdate{
		\par\end{center}
		\begin{figure}[h!]
			\centering \includegraphics[width=15cm]{#1}
			\caption{#2}
		\end{figure}
	}
}
...
Large input data sets remove the ability to search all possible combinations of moves, meaning that a either a search depth needs to be implemented or a maximum search time.
...
This allowed us to evaluate the strength an weaknesses of each language - an analysis which would have to be conducted if the code was to be hard optimized for production.
...
Unfortunately this means a maximum tile size of 12288 which we predicted would be readily achievable by a fully optimized algorithm. Bitboards are optimal because they provide the CPU with a memory unit that it can make extremely efficient shift and addition operations on.
Unfortunately this advantage is largely lost when using multiple boards or 128 bit boards are used -- as would be required for the tile size -- especially on machines with 32 bit registers.
We opted to use more portable code here.

\subsection*{Naive}
\paragraph \indent
Initially an naive algorithm was implemented to play Threes one move at a time.
...
A major difference between 2048 and Threes is that in 2048 the next tile is placed randomly, while in Threes the location of the next tile will be placed in the lexicographical lowest location.
